import React from "react";

export default function RiskBarVertical({ hf }: { hf: number }) {
  const clamped = Math.max(0.8, Math.min(3.0, hf));
  const bottomPct = ((3.0 - clamped) / (3.0 - 0.8)) * 100;

  return (
    <div className="relative h-full w-3 rounded-full bg-white/10" aria-label={`Risk scale, marker at ${Math.round(bottomPct)}%`}>
      {/* gradient track */}
      <div className="absolute inset-0 rounded-full bg-gradient-to-b from-rose-500 via-yellow-300 to-teal-400 opacity-80" />
      {/* marker */}
      <div
        className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full ring-2 ring-white/60 shadow transition-all duration-500"
        style={{ bottom: `${bottomPct}%`, background: "white" }}
      />
      {/* tick labels */}
      <div className="absolute inset-x-0 text-[10px] leading-none text-white/60 -left-8 hidden sm:block">
        <span className="absolute top-0">High</span>
        <span className="absolute top-1/2 -translate-y-1/2">Mid</span>
        <span className="absolute bottom-0">Low</span>
      </div>
    </div>
  );
}
